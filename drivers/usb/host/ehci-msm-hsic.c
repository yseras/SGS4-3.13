...
#define	HCD_RT_OLD_ENUM	0x0008		/* HC supports short enumeration
					   on root port; originally defined in hcd.h */
...
static int msm_hsic_reset_done(struct usb_hcd *hcd)
{
	...
	if (ret)
		pr_err("reset ehci_handshake failed in %s\n", __func__);
	else
...
static void ehci_hsic_reset_sof_bug_handler(struct usb_hcd *hcd, u32 val)
{
	...
	ret = ehci_handshake(ehci, &ehci->regs->status, STS_HALT,
			STS_HALT, 16 * 125);
	if (ret) {
		pr_err("halt ehci_handshake fatal error\n");
...
retry:
	...
	if (ehci_handshake(ehci, status_reg, PORT_RESET, 0, 10 * 1000)) {
		pr_err("reset ehci_handshake fatal error\n");
				...
				struct platform_device *pdev)
{
	...
	pdata->consider_ipa_handshake = (of_property_read_bool(node,
				"hsic,consider-ipa-ehci_handshake"));
